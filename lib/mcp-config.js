...
// `mcpExtras` block name keeps the user-facing language (`--browser`) decoupled
// from the config key (`browser`) — useful if we ever want aliases.
const EXTRA_FLAG_TO_NAME = Object.freeze({
  '--browser': 'browser',
});

export function knownMcpExtras() {
  return Object.values(EXTRA_FLAG_TO_NAME);
}
...
export function extractMcpExtraFlags(tokens) {
  ...
  for (const tok of tokens) {
    const mapped = EXTRA_FLAG_TO_NAME[tok];
