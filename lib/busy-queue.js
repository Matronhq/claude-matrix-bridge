...
// path), while the Matrix-only notification edits
// (stripQueueNotificationLinks, editMessage, and the sendHtml sink) are
// simply not passed by the journal caller: the journal protocol has no
// message editing, so its feedback is a fresh plain text and the "📨 Queued"
// Matrix tiles are left untouched — a stale tile in the room beats crashing
// or editing the wrong event.
...
//   'cancel': pop the LAST queued message; with an editMessage seam, also
//             pop its notification and edit it to "✕ … (cancelled)" (a
//             notification without an eventId is popped but not edited —
//             same guard the Matrix branch had); reply with the exact
//             remaining-count strings.
export async function handleBusyQueueMagicWord(session, action, {
  ...
  queue.pop();
  if (editMessage && notifs.length > 0) {
